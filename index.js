...
console.log('db initializeds');
await db.migrate();
app.get('/', async function (req, res) {
    const fetchAllPlans = await db.all(`select * from price_plan`);
    res.json({
        price_plans: fetchAllPlans
    })
})
...
app.post('/api/price_plan/update', async function (req, res) {
    ...
    const { price_plan, sms_price, call_price } = req.body;
    await db.all(`update price_plan set sms_price = ?, call_price = ? where plan_name = ?`, sms_price, call_price, price_plan)
});
